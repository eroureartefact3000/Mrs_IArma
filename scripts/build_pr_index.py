"""Build the vector index for the PR 2024 winners.

Reads per-board extractions from `data_internal/extractions/pr/` and per-winner
rationales from `data_internal/rationales/pr/`, merges them, embeds via Voyage,
and writes:

    data/pr_index.npy           — float32 matrix (n_winners × 1024)
    data/pr_index_meta.jsonl    — one JSON line per board (in matrix-row order)

Only boards with a rationale (= medal winners) are indexed. Shortlists are
intentionally excluded — they lack a tier-justified "why it won" anchor.

Usage:
    uv run python scripts/build_pr_index.py
"""
...
EXTRACTIONS_DIR = ROOT / "data_internal" / "extractions" / "pr"
RATIONALES_DIR = ROOT / "data_internal" / "rationales" / "pr"
DATA_DIR = ROOT / "data"
EMBEDDINGS_PATH = DATA_DIR / "pr_index.npy"
META_PATH = DATA_DIR / "pr_index_meta.jsonl"


def load_merged_records() -> list[dict]:
    """Load extractions and attach rationale as `why_it_won` for winners."""
    records: list[dict] = []
...
def main() -> None:
    ...
    assert records, "No PR winners found — aborting."
