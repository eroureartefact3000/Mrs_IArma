"""Generate the dispatching inventory for the PR pilot.

Walks 2024/PR/{GRAND PRIX, GOLD, SILVER, BRONZE, SHORTLIST}/ and produces:
  - data_internal/pr_inventory.json — a single source of truth for the 115 boards
...
PR_DIR = ROOT / "2024" / "PR"
OUT_PATH = ROOT / "data_internal" / "pr_inventory.json"

# Folder → canonical Tier value (must match Tier literal in pipeline.schema)
FOLDER_TO_TIER = {
...
def slug_of(filename: str) -> str:
    """Canonical slug rule (see agent_briefs/02_OUTPUT_LOCATIONS.md)."""
    stem = Path(filename).stem.lower()
    # Replace problematic chars with hyphen
    for ch in (" ", "–", "(", ")", "/", "_", ",", ".", "'", "’", "&", "?"):
...
def make_batches(boards: list[dict], n_batches: int) -> list[list[dict]]:
    """Stratified round-robin so each batch has a similar tier mix."""
    # Sort by tier (so we round-robin within tier order) then by filename
...
def check_duplicate_slugs(boards: list[dict]) -> list[tuple[str, list[dict]]]:
    """Slug collisions would break the file-naming convention."""
...
def deduplicate_keep_highest_tier(boards: list[dict]) -> list[dict]:
    """A campaign sometimes appears in multiple tier folders (same campaign
    entered in several PR sub-categories at different tier outcomes).
    The image is identical — only the categorisation differs. We dedupe by
    slug and keep the entry with the highest tier rank.
    """
    ...
    for b in boards:
        ...
        if existing is None:
            ...
            if _TIER_RANK.get(b["tier"], 0) > _TIER_RANK.get(existing["tier"], 0):
                best_by_slug[slug] = b
    # Attach also_won_tiers metadata to deduplicated records
    out: list[dict] = []
...
def main() -> None:
    ...
    parser.add_argument("--batches", type=int, default=5, help="number of subagent batches to plan")
    args = parser.parse_args()

    boards = collect_boards()
    print(f"Total PR boards: {len(boards)}")
    print()
    ...
    if dups:
        print(f"\n⚠️  {len(dups)} duplicate slugs detected (same campaign in multiple PR sub-categories).")
        print("   Deduplicating: keeping the entry at the highest tier for each campaign.")
        ...
        for t in FOLDER_TO_TIER.values():
            print(f"     {t:11s}: {counts.get(t, 0)}")
        # Show what we kept
        print("   Multi-placement campaigns and their kept tier:")
        for b in boards:
